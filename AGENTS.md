# AGENTS.md
这个项目是 AI 控制了一个独立的 Minecraft 机器人。而不是玩家在使用 AI 控制自己。请明确区分这一点。
我们需要做出一个**像真人玩家一样**的 AI 玩家。它可以聊天，也可以移动、拿取物品、装备物品等等。

## 核心信条
- 内部实现无需顾虑向后兼容；我们只在意最快速交付可以上线的行为。
- 一切设计以“让 AI 获取问题所需的上下文最少”为目标：结构即文档，状态即真相，杜绝散落的隐式依赖。避免重复代码到处拷贝，始终保证 DRY (Don' Repeat Yourself) 原则
