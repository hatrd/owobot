...
验证：pathfinder 集成版本 lint 和 97 个测试通过；独立空目录 `npm ci` 成功，`npm ls mineflayer-pathfinder prismarine-item` 确认所有物品库实例统一到 fork 提交。落差用例和同步取消的 8 个用例均记录了上游基线失败、修复后通过。项目仍保留本地导航的水域风险策略及挖掘保护；它们是游戏策略，不属于上游通用修复。

集成分支另 cherry-pick 现有上游 [#369](https://github.com/PrismarineJS/mineflayer-pathfinder/pull/369) 的 `655875f`、`48686a8`，保留原作者：修复 A* 最小堆右子节点边界和 GoalCompositeAll 的最大值初始值。独立复现 `[0,5,1,7]` 入堆在旧版弹出 `[0,5,1,7]`，修复后按 `[0,1,5,7]` 弹出。已有 PR，不重复提交。
