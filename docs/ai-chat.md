...
- 活跃窗口内：若玩家继续说普通聊天，先进入静默合并窗口，默认 5s（可用 `state.ai.followupDelayMs` 覆盖）后触发一次“跟进调用”；窗口内多句 followup 会合并成一次 `main_chat`，并在发起外部请求前复用 `state.ai.limits` 频率门，避免一句一调用或纯 followup 绕过限流。停止/下坐/记忆/忘记等本地高优先级命令仍立即处理。
- 触发词重新出现延长会话
