...
test('casual Chinese chat containing 哪有 stays on lightweight chat profile', async () => {
  ...
  const harness = makeExecutor({
    recent,
    gameText: repeatedText('不该注入的位置游戏状态', 6000),
    memory: makeMemory(repeatedText('普通长期记忆', 1000)),
    peopleText: repeatedText('普通画像', 900)
  })
  try {
...
test('local stats tool result halts without a second model call', async () => {
  const recent = makeRecentFromLogShape({ day: '2026-01-31', count: 10, chars: 80 })
  const harness = makeExecutor({
    recent,
    assistantMessages: [
        ...
        tool_calls: [
          {
            ...
            function: {
              ...
              arguments: JSON.stringify({ name: 'Alice', type: 'all' })
            }
          }
        ]
      },
  ...
  try {
    const res = await harness.executor.callAI(
      'kuleizi',
      'owkowk 查一下 Alice 的统计',
      { topic: 'stats', kind: 'chat' },
      { inlineUserContent: true, contextProfile: 'task' }
    )
    assert.equal(harness.calls.length, 1, `expected local stats tool to avoid follow-up LLM call, got ${harness.calls.length}`)
    assert.equal(res.reply, '')
    assert.equal(harness.sent.length, 1)
    assert.match(harness.sent[0].text, /ok|完成/)
  } finally {
...
test('read-only observe query halts without a second model call', async () => {
  ...
  try {
    const res = await harness.executor.callAI(
      'kuleizi',
      'owkowk 看看附近有什么实体',
      { topic: 'observe', kind: 'chat', nearby: true },
      { inlineUserContent: true, contextProfile: 'task' }
    )
    assert.equal(harness.calls.length, 1, `expected read-only observe query to avoid follow-up LLM call, got ${harness.calls.length}`)
    assert.equal(res.reply, '')
    assert.equal(harness.sent.length, 1)
    assert.match(harness.sent[0].text, /ok|完成/)
  } finally {
...
test('voice speak tool result halts without a second model call', async () => {
  const recent = makeRecentFromLogShape({ day: '2026-01-31', count: 10, chars: 80 })
  const harness = makeExecutor({
    recent,
    assistantMessages: [
        ...
        tool_calls: [
          {
            ...
            function: {
              name: 'voice_speak',
              arguments: JSON.stringify({ preset: 'ciallo' })
            }
          }
        ]
      },
  ...
  try {
    const res = await harness.executor.callAI(
      'kuleizi',
      'owkowk ciallo 一下',
      { topic: 'voice', kind: 'chat' },
      { inlineUserContent: true, contextProfile: 'task' }
    )
    assert.equal(harness.calls.length, 1, `expected voice speak tool to avoid follow-up LLM call, got ${harness.calls.length}`)
    assert.equal(res.reply, '')
    assert.equal(harness.sent.length, 1)
    assert.match(harness.sent[0].text, /ok|完成/)
  } finally {
...
test('control and skill state tools halt without a second model call', async () => {
  const recent = makeRecentFromLogShape({ day: '2026-01-31', count: 10, chars: 80 })
  const cases = [
    { tool: 'reset', args: {}, prompt: 'owkowk 重置一下' },
    { tool: 'stop', args: {}, prompt: 'owkowk 停下' },
    { tool: 'stop_all', args: {}, prompt: 'owkowk 全部停止' },
    { tool: 'skill_status', args: { taskId: 'task_1' }, prompt: 'owkowk 查一下任务 task_1 状态' },
    { tool: 'skill_cancel', args: { taskId: 'task_1' }, prompt: 'owkowk 取消任务 task_1' }
  ]
...
test('deterministic read-only result tools halt in action context without a second model call', async () => {
  const recent = makeRecentFromLogShape({ day: '2026-01-31', count: 10, chars: 80 })
  const cases = [
    { tool: 'read_book', args: { list: true }, prompt: 'owkowk 读一下背包里的书' },
    { tool: 'voice_status', args: {}, prompt: 'owkowk 检查语音状态' }
  ]
