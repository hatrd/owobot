...
test('auto-back waits for respawn+full-heal after prompt before /back', async () => {
  const chats = []
...
test('auto-back retries /back after teleport cancel (once ready) even within cooldown', async () => {
  const chats = []
  const handlers = new Map()
  ...
  try {
    ...
    await new Promise(resolve => setTimeout(resolve, 1600))
    assert.equal(chats.filter(x => x === '/back').length, 1)
