  ...
  function run (tool, args) {
    ...
    return Promise
      ...
      .then(() => {
        ...
        if (!validation.ok) return validation
        if (!['controller_read', 'controller_write', 'life_configure'].includes(tool) && getToolMetadata(tool)?.dryCapability !== 'read_only') bot.state?.lifeApi?.yield('player_action')
        const lease = bot.state?.controller?.lease
        const isController = tool === 'controller_read' || tool === 'controller_write' || tool === 'life_configure'
