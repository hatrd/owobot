...
- AI function tool 列表由 `TOOL_SPECS` 自动派生；`bot_impl/action-tool-schemas.js` 提供参数 schema 与校验，dry/run/AI 共用。
...
3. 在 `bot_impl/action-tool-schemas.js` 增加该工具参数 schema（必填；缺失会阻止加载）。
...
## 建造工具
- `modules/building.js` 提供真实配方预览、显式合成与新告示牌放置/文字回读。放牌失败必须保留已执行阶段，不能把未确认写字当成功，也不能盲目重复放置。
