...
function scanFrames (bot, radius = 12) {
  ...
  const clonePos = (v) => ({ x: v.x, y: v.y, z: v.z })
  ...
  const chestPositions = bot.findBlocks({
    point: center,
    maxDistance: clampRadius,
    count: 256,
    matching: (block) => {
      try { return block && chestNames.has(String(block.name || '').toLowerCase()) } catch { return false }
    }
  }) || []
  ...
  for (const bc of chestPositions) {
    try {
      const block = bot.blockAt(bc)
      const info = {
        name: block?.name || null,
        position: { x: bc.x, y: bc.y, z: bc.z },
        distance: round(bc.distanceTo(center), 2)
      }
      const key = posKey(bc)
      let frame = frameAttachMap.get(key)
      if ((!frame || usedFrames.has(frame.id)) && block?.name === 'chest') {
        // For double chests, try neighbor frame attachments pointing to same large chest
        const neighbors = [new Vec3(1, 0, 0), new Vec3(-1, 0, 0), new Vec3(0, 0, 1), new Vec3(0, 0, -1)]
        for (const off of neighbors) {
          const neighborPos = bc.plus(off)
          const neighborBlock = bot.blockAt(neighborPos)
          if (!neighborBlock || neighborBlock.name !== block?.name) continue
          const neighborKey = posKey(neighborPos)
          const candidate = frameAttachMap.get(neighborKey)
          if (candidate && !usedFrames.has(candidate.id)) {
            frame = candidate
            break
          }
        }
      ...
      if (!frame) {
        // Fallback: approximate by nearest frame if very close (legacy heuristic)
        const centerChest = new Vec3(bc.x + 0.5, bc.y + 0.5, bc.z + 0.5)
        let best = null
        let bestScore = Infinity
        for (const candidate of frames) {
          if (!candidate.itemKey) continue
          if (usedFrames.has(candidate.id)) continue
          const fp = new Vec3(candidate.position.x, candidate.position.y, candidate.position.z)
          const diff = fp.minus(centerChest)
      ...
      if (frame && frame.itemKey && !usedFrames.has(frame.id)) {
        framedChests.push({ ...info, frame, itemKey: frame.itemKey })
        framedKeys.add(key)
        usedFrames.add(frame.id)
      } else {
        unframedChests.push(info)
      }
    } catch {}
  }
...
function install (bot, { on, state, log }) {
  ...
  const ensureNear = async (pos, actions, range = 2.5) => {
    const target = new Vec3(pos.x + 0.5, pos.y + 0.5, pos.z + 0.5)
    try {
      const here = bot.entity?.position
      if (here && here.distanceTo(target) <= range) return true
    } catch {}
    const res = await actions.run('goto', { x: pos.x + 0.5, y: pos.y, z: pos.z + 0.5, range })
    ...
    try {
      const here = bot.entity?.position
      return here && here.distanceTo(target) <= range
    } catch { return false }
  }
  ...
  async function processSourceChest (source, destMap, actions) {
    ...
    while (true) {
      ...
      const takeCount = targetItem.count || 1
      try {
      ...
      for (const dest of wanted) {
        ...
        if (!await ensureNear(dest.position, actions)) {
          logger.warn('[frame-sort] 前往目标箱子失败', dest.position)
          continue
        }
        let destContainer
        try {
          destContainer = await openContainerAt(dest.position)
        } catch (err) {
  ...
  function handleCli (args = []) {
    ...
    if (sub === 'scan' || sub === 'list') {
      ...
      scan.framedChests.forEach((c) => {
        ...
        console.log(`  [FRAME] ${c.position.x},${c.position.y},${c.position.z} -> ${it?.displayName || it?.name || '未知'} (key=${c.itemKey})`)
      })
